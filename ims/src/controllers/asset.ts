...
export function makeFindAsset(findSignageAsset: any) {
  return async function findAsset(httpRequest: any): Promise<HttpResponse> {
    ...
    try {
      const asset = await findSignageAsset(params.id);
      ...
      data.attributes.breadth = asset.breadth;
      data.attributes.depth = asset.depth;
      if (asset.allocation && asset.allocation.centreID && asset.allocation.locationID && asset.allocation.code) {
        data.attributes.allocation = {};
        data.attributes.allocation.centreId = asset.allocation.centreID;
        data.attributes.allocation.locationId = asset.allocation.locationID;
        data.attributes.allocation.code = asset.allocation.code;
      }

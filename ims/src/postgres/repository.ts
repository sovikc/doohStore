...
export function makeRepository(pool: any) {
  const storeCentre = async (centre: any, userID: string) => {
    ...
    try {
      ...
      const shoppingCentreNumericID = res.rows[0].shopping_centre_id;
  ...
  const updateCentre = async (centre: any, userID: string) => {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      await client.query('SET TRANSACTION ISOLATION LEVEL READ COMMITTED');
      const updateCentreStmt = `UPDATE shopping_centre SET shopping_centre_name = $1, 
      address_line_one = $2, address_line_two = $3, city = $4, state_name = $5, postcode = $6, country = $7, created_by = $8 
      WHERE shopping_centre_uuid = $9 RETURNING shopping_centre_id`;
      const res = await client.query(updateCentreStmt, [
        centre.name,
        centre.address.lineOne,
        centre.address.lineTwo,
        centre.address.city,
      ...
      await client.query('COMMIT');
      return shoppingCentreNumericID;
    } catch (err) {
      await client.query('ROLLBACK');
      err.name = Errors.RuntimeError;
      throw err;
  ...
  const removeCentre = async (centreID: string, userID: string) => {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      await client.query('SET TRANSACTION ISOLATION LEVEL READ COMMITTED');
      const deactivateCentreStmt = `UPDATE shopping_centre SET deactivated = $1 where shopping_centre_uuid = $2 RETURNING shopping_centre_id`;
      const res = await client.query(deactivateCentreStmt, [true, centreID]);
      const shoppingCentreNumericID = res.rows[0].shopping_centre_id;

      const deactivateLocationStmt = `UPDATE location_within_centre SET deactivated = $1 where shopping_centre_uuid = $2`;
      await client.query(deactivateLocationStmt, [true, centreID]);

      const deleteAllocationStmt = `DELETE FROM asset_allocation where shopping_centre_uuid = $1`;
      await client.query(deleteAllocationStmt, [centreID]);
  ...
  const findLocationByID = async (locationID: string) => {
    const client = await pool.connect();
    let location: any = null;

    const selectLocationStmt = `SELECT * from location_within_centre where location_uuid = $1 and deactivated = $2`;
    const values = [locationID, false];
    ...
    try {
      ...
      if (!row) {
        ...
        return { location, nonExistentID };
      }

      location = makeLocation(row.location_code, row.location_uuid);
      const matchingCentreID = row.shopping_centre_uuid;
      return { location, matchingCentreID };
    } catch (e) {
      const err = e;
      err.name = Errors.RuntimeError;
      throw err;
  ...
  const updateLocation = async (location: any, userID: string) => {
    ...
    try {
      await client.query('BEGIN');
      await client.query('SET TRANSACTION ISOLATION LEVEL READ COMMITTED');
      const updateLocationStmt = `UPDATE location_within_centre SET location_code = $1, created_by = $2 WHERE location_uuid = $3 RETURNING location_id`;
      const res = await client.query(updateLocationStmt, [location.code, userID, location.id]);
      const locationNumericID = res.rows[0].location_id;
      const insertLogStmt = `INSERT INTO change_log(entity_id, entity_type, operation_type, field_name, field_value_type, field_char_value, created_by)
      VALUES($1, $2, $3, $4, $5, $6, $7)`;
      await client.query(insertLogStmt, [
        locationNumericID,
  ...
  const removeLocation = async (centreID: string, locationID: string, userID: string) => {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      await client.query('SET TRANSACTION ISOLATION LEVEL READ COMMITTED');
      const updateLocationStmt = `UPDATE location_within_centre SET deactivated = $1, created_by = $2 WHERE location_uuid = $3 and shopping_centre_uuid = $4 RETURNING location_id`;
      const res = await client.query(updateLocationStmt, [true, userID, locationID, centreID]);
      const locationNumericID = res.rows[0].location_id;

      const deleteAllocationStmt = `DELETE FROM asset_allocation where shopping_centre_uuid = $1 and location_uuid = $2`;
      await client.query(deleteAllocationStmt, [centreID, locationID]);
  ...
  const findAssetByID = async (assetID: string) => {
    ...
    try {
      ...
      const allocationRecord = {
        centreID: row.shopping_centre_uuid,
      ...
      if (allocationRecord.centreID && allocationRecord.locationID && allocationRecord.code) {
  ...
  const removeAllocation = async (centreID: string, locationID: string, assetID: any, userID: string) => {
    ...
    try {
      await client.query('BEGIN');
      await client.query('SET TRANSACTION ISOLATION LEVEL READ COMMITTED');
      const deleteAllocationStmt = `DELETE FROM asset_allocation where shopping_centre_uuid = $1 and location_uuid = $2 and asset_uuid = $3 RETURNING allocation_id`;
      const res = await client.query(deleteAllocationStmt, [centreID, locationID, assetID]);
      const allocationID = res.rows[0].allocation_id;
      const insertLogStmt = `INSERT INTO change_log(entity_id, entity_type, operation_type, field_name, field_value_type, field_char_value, created_by)
      VALUES($1, $2, $3, $4, $5, $6, $7)`;
      await client.query(insertLogStmt, [

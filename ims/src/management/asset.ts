...
export function makeManageAssets(repository: Repository) {
  ...
  async function findAsset(assetID: string) {
    if (!assetID) {
      const err = new Error(ErrorMessages.ValidAssetID);
  ...
  async function updateAsset(
    ...
    if (name && name !== asset.name) {
      ...
      if (existingAsset) {
        const err = new Error(ErrorMessages.AssetExists);
        err.name = Errors.ConflictError;
        throw err;
      }
      asset.name = name;
  ...
  async function allocateAsset(userID: string, id: string, centreID: string, code: string) {
    ...
    if (!location) {
      const err = new Error(ErrorMessages.NonExistentLocationInCentre);
      err.name = Errors.InvalidRequestError;
      throw err;
    }
    ...
    const numericID = await repository.storeAllocation(centreID, location.id, id, userID).catch((err) => {
      throw err;
    });
    ...
    const newAllocation = { centreID, locationID: location.id, code };
  ...
  return { createAsset, findAsset, allocateAsset, deallocateAsset, updateAsset };
}
